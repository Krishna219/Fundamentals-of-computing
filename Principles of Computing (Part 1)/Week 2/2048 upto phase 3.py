...
class TwentyFortyEight:
    ...
    def __init__(self, grid_height, grid_width):
        ...
        #to merge function
        self.initial_tiles = { UP : [(0, num) for num in range(self.grid_width)],
                        DOWN : [(self.grid_height - 1, num) for num in range(self.grid_width)],
                        LEFT : [(num, 0) for num in range(self.grid_height)],
                        RIGHT : [(num, self.grid_width - 1) for num in range(self.grid_height)]}
    ...
    def move(self, direction):
        ...
        if (direction == UP) or (direction == DOWN):
            num_tiles = self.grid_height
            
        #num_tiles for horizontal slide is grid_width
        elif (direction == LEFT) or (direction == RIGHT):
            num_tiles = self.grid_width
        
#        print self.grid
        
        for cell in self.initial_tiles[direction]:
...
#        print self.grid
    ...
    def new_tile(self):
        ...
        self.grid[chosen_grid[0]][chosen_grid[1]] = random.choice(new_tiles)
