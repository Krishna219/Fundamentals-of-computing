...
def strategy_best(cookies, cps, history, time_left, build_info):
    ...
    """
    affordable_expensive_item = strategy_expensive(cookies, cps, history, time_left, build_info)
#    print affordable_expensive_item, time_left, len(history)
    
    cheapest_item = strategy_cheap(cookies, cps, history, time_left, build_info)
   
    build_details = build_info.clone()    
    
    cheap_game = simulate_clicker(build_details, time_left, strategy_cheap)
    expensive_game = simulate_clicker(build_details, time_left, strategy_expensive)
    
    cheap_history = cheap_game.get_history()
    expensive_history = expensive_game.get_history()
    
    
    print
    print time_left
    
    print cheap_game.get_cps() / expensive_game.get_cps(), 
    print cheap_history[len(cheap_history) -1][3] + cheap_game.get_cookies()
    if len(expensive_history) > 1:
        print expensive_history[1]
#    growth_rate = (cheap_history[
            
#    history = my_game.get_history()
    print cheap_game
    print expensive_game
    
#    strategy_game = simulate_clicker(build_details, history[1][0], strategy_cheap)
#    print strategy_game
    
    if time_left <= (0.4 * 600):
#        print affordable_expensive_item
        return affordable_expensive_item
    else:
        print cheapest_item
        return cheapest_item
...
#    simpleplot.plot_lines(strategy_name, 400, 400, 'Time', 'Total Cookies', [history], True)
    return history
