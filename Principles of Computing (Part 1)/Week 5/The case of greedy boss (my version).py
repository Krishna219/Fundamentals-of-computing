...
def greedy_boss(days_in_simulation, bribe_cost_increment, plot_type = STANDARD):
    ...
    while current_day <= days_in_simulation:
        ...
        if current_account >= current_bribe_cost:
            current_account = current_account - current_bribe_cost
            current_salary = current_salary + SALARY_INCREMENT
            current_bribe_cost = current_bribe_cost + bribe_cost_increment
        
        # advance current_day to day of next bribe (DO NOT INCREMENT BY ONE DAY)
        while current_account < current_bribe_cost:
            current_day = current_day + 1
            current_account = current_account + current_salary
            total_salary_earned = total_salary_earned + current_salary
