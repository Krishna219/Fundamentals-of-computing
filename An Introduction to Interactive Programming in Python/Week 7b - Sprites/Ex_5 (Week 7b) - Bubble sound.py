...
class Bubble:
    ...
    def update(self):
        ...
        if self.pos[0] < BUBBLE_RADIUS or self.pos[1] > WIDTH - BUBBLE_RADIUS:
    ...
    def draw(self, canvas):
        canvas.draw_circle(self.pos, BUBBLE_RADIUS, 1, "cyan", self.color)
     

# define keyhandlers to control firing_angle
def keydown(key):
    global a_bubble, firing_angle_vel, bubble_stuck, firing_sound
    if simplegui.KEY_MAP['space'] == key:
        vel = angle_to_vector(firing_angle)
        a_bubble.fire_bubble([math.pi * vel[0], - math.pi * vel[1]])
        firing_sound.play()
...
def draw(canvas):
    ...
    canvas.draw_line(FIRING_POSITION, upper_endpoint, 4, "rgb(100, 255, 100)")
